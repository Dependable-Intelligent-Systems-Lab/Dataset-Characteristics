(function () {
  ...
  function pageTitle() {
    ...
    return cleanTitle(document.title.replace(/[-\u2014]\s*D-ACE.*$/, ""));
  }
  ...
  function createLink(provider, icon, label) {
    ...
    link.className = "dace-ai-panel__link";
    link.href = providerUrl(provider);
    link.target = "_blank";
    link.rel = "noopener noreferrer";

    var iconNode = document.createElement("span");
    iconNode.className = "dace-ai-panel__icon";
  ...
  function injectPanel() {
    ...
    if (!sidebar || sidebar.querySelector(".dace-ai-panel")) {
      return;
    }

    var panel = document.createElement("div");
    panel.className = "dace-ai-panel";

    var title = document.createElement("p");
    title.className = "dace-ai-panel__title";
