---
title: D-ACE Dataset Characteristics
description: Assess dataset structure, sparsity, correlations, intrinsic dimensionality, feature noise, and class covariance signals before training machine learning models.
keywords: D-ACE, dataset characteristics, dataset quality, trustworthy AI, dependable AI, machine learning, Read the Docs
---
...
D-ACE helps researchers and machine learning engineers inspect the quality and
structure of tabular datasets before they train models. It reports compact,
repeatable characteristics such as dimensionality, sparsity, class count,
feature correlation, multivariate normality, intrinsic dimensionality, feature
noise, and homogeneity of class covariance.

Use it as an early dataset assessment step when you need to understand whether
data shape, missingness, class structure, or feature relationships may affect
model dependability.

:::{tip}
Run D-ACE before model selection. A short characteristic report can reveal
high sparsity, unexpected class structure, or correlated features before those
issues become harder-to-debug model behavior.
:::
...
:::{grid-item-card} Dataset Structure

Measure the number of features, instances, and classes to understand the
basic shape of a dataset.
+++
Functions include `getDatasetDimensionality`, `getDatasetNumberOfInstances`,
and `getDatasetNumberOfClasses`.
:::

:::{grid-item-card} Sparsity and Missingness

Inspect zero sparsity and NaN sparsity so preprocessing decisions are based on
observable data quality signals.
+++
Functions include `getZeroSparsity`, `getNaNSparsity`, and per-feature
zero/NaN counters.
:::

:::{grid-item-card} Feature Relationships

Summarize feature correlation with and without class labels, then check
multivariate normality for the feature space.
+++
Functions include `getCorrelationOfFeaturesWithClass`,
`getCorrelationOfFeaturesWithNoClass`, and `getMultiVariateNormality`.
:::

:::{grid-item-card} Dimensionality and Noise

Estimate intrinsic dimensionality with PCA-based methods and compare feature
noise signals across datasets.
+++
Functions include `getIntrinsicDimensionaltiy`,
`getIntrinsicDimensionaltiyRatio`, `getFeatureNoise`, and `getFeatureNoise2`.
:::
...
:::{code-block} python
:caption: Generate a D-ACE characteristic report
...
Functions that compare features with labels require the class column name via
